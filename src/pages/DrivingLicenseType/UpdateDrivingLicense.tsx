...
import {
  ...
  getByIdDrivingLicenseType,
  updateDrivingLicenseType,
} from "../../store/slices/drivingLicenseTypeSlice";
...
const UpdateDrivingLicenseType = () => {
  ...
  const [drivingLicenseType, setDrivingLicenseType] =
    useState<DrivingLicenseTypeModel>();
  ...
  const fetchData = async () => {
    try {
      const newResponse = await dispatch(
        getByIdDrivingLicenseType({ id: drivingLicenseTypeId })
      );
  ...
  const handleUpdateDrivingLicenseType = async (values: any) => {
    try {
      ...
      window.location.href = "/adminPanel/DrivingLicenseTypes";
    } catch (error) {
  ...
  return (
    <SideBar>
      ...
      >
        <Title style={{ marginLeft: "35%" }}>Ehliyet Tipi Güncelleme</Title>
        <Formik
