    ...
    useEffect(() => {
        if (rentalState.rentals && customerState.customers && carState.cars && rentalStatusState.rentalStatuses) {
            const tableData = rentalState.rentals.map((rental: any) => [
                ...
                <Button  style={{backgroundColor:'rgba(140, 25, 25)'}} onClick={() => handleStartUpdate(rental.id)}>Başlat</Button>,
                <Button  style={{backgroundColor:'rgba(140, 25, 25)'}} onClick={() => handleReturnUpdate(rental.id)}>Bitir</Button>,
            ]);
